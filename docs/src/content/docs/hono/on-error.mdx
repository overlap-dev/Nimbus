---
title: onError Handler
description: >-
  Hono onError handler that maps EventFabric exceptions to structured JSON HTTP
  responses with correct status codes.
---
import InDepthExampleCard from '../../../components/InDepthExampleCard.astro';

The `handleError` function is an error handler for Hono applications that converts EventFabric exceptions to structured HTTP JSON responses.
...
When a EventFabric exception is thrown, the handler returns a JSON response with the following structure:
...
| Unhandled   | `critical` | Non-EventFabric errors, unexpected failures |
