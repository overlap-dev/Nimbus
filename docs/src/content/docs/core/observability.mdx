...
title: Observability
description: >-
  Built-in logging, tracing, and metrics via OpenTelemetry with zero boilerplate
  on routers, handlers, and messages.
---
import InDepthExampleCard from '../../../components/InDepthExampleCard.astro';

Observability is a first-class citizen in EventFabric. The framework is designed so that developers can focus on business logic without implementing logging, tracing, and metrics from the ground up.
...
The three pillars of observability in EventFabric:
...
EventFabric uses the [OpenTelemetry API](https://opentelemetry.io/) (`@opentelemetry/api`) for all observability instrumentation. This provides:
...
EventFabric builds upon [Deno's native OpenTelemetry support](https://docs.deno.com/runtime/fundamentals/open_telemetry/). With Deno 2.x, you can enable OTEL export with zero additional dependencies.
...
All traces, metrics, and logs from EventFabric will automatically be exported to your configured backend.

## Built-in Instrumentation

### MessageRouter

The [MessageRouter](/core/router) automatically creates spans for every routed message:
...
The [EventFabricEventBus](/core/event-bus) instruments both publishing and handling:
...
The [Logger](/core/logging) outputs structured logs to the console. When combined with Deno's OTEL support, logs are automatically exported alongside traces and metrics.
...
All messages in EventFabric (Commands, Queries, Events) carry a `correlationid` field. This enables:
