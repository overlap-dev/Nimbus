...
title: Queries
description: >-
  Queries are CloudEvents-shaped read messages that fetch data without mutating
  application state.
---
...
A query in EventFabric follows the CloudEvents specification and consists of:
...
EventFabric provides a base Zod schema for validating queries:
...
Queries are routed to handlers using the [MessageRouter](/core/router). See the Router documentation for details on registering handlers and routing messages.
