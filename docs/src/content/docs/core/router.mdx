...
title: Router
description: >-
  MessageRouter routes commands, queries, and events to handlers with Zod
...
| Handler error      | *(propagated)*          | Errors from handlers are propagated unchanged |

## Observability

The router is fully instrumented with OpenTelemetry tracing and metrics. See the [Observability](/core/observability) documentation for details.
