...
title: Commands
description: >-
  Commands are CloudEvents-shaped write messages expressing intent to change
  state; validated and routed to handlers.
---
...
A command in EventFabric follows the CloudEvents specification and consists of:
...
EventFabric provides a base Zod schema for validating commands:
...
Commands are routed to handlers using the [MessageRouter](/core/router). See the Router documentation for details on registering handlers and routing messages.
